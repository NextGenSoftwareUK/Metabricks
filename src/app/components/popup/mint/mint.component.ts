...
export class MintComponent implements OnInit {
  ...
  async mintBrickArbitrum() {
    ...
    try {
      ...
      const response = await fetch('https://metabricks-backend-api-v2-42ff9579046d.herokuapp.com/api/mint-nft', {
        method: 'POST',
        headers: {
  ...
  async processStripePayment() {
    ...
    try {
      ...
      const response = await fetch('https://metabricks-backend-api-v2-42ff9579046d.herokuapp.com/create-checkout-session', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async checkPaymentStatus(sessionId: string) {
    ...
    const pollStatus = async () => {
      try {
        const response = await fetch(`https://metabricks-backend-api-v2-42ff9579046d.herokuapp.com/check-payment-status/${sessionId}`);
