...
export class OasisApiService {
  ...
  private readonly OASIS_API_CONFIG = {
    ...
    ENDPOINTS: {
      ...
      AVATAR: '/avatar',
      SOLANA_MINT: '/Solana/Mint', // Updated to match our working endpoint
      NFT_GET: '/nft',
  ...
  /**
   * Authenticate user with OASIS API using proper JWT authentication
   * Note: This method is now handled by the AvatarService for better integration
   */
  async authenticate(email: string, password: string): Promise<boolean> {
    try {
      console.log('🔐 Attempting to authenticate with OASIS API:', { email });
      
      // Use proper JWT authentication endpoint
      const response = await fetch(`${this.OASIS_API_CONFIG.BASE_URL}/auth/login`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success && result.jwtToken) {
          ...
          this.avatarId = result.user?.id || result.avatarId;
          ...
          console.log('✅ OASIS JWT authentication successful');
          return true;
        } else {
          console.log('❌ OASIS authentication failed: Invalid response format');
          return false;
        }
      } else {
        const errorData = await response.json();
        console.log('❌ OASIS authentication failed:', errorData.message || `HTTP ${response.status}`);
        return false;
      }
    } catch (error) {
  ...
  async mintNFT(nftData: OASISNFTMintRequest): Promise<OASISNFTMintResponse> {
    ...
    try {
      const response = await fetch(`${this.OASIS_API_CONFIG.BASE_URL}${this.OASIS_API_CONFIG.ENDPOINTS.SOLANA_MINT}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
